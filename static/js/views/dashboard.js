...
function renderDashboard() {
  ...
  // Filter other data based on filtered employees
  ...
  const asistentesHoy = filteredAsistencias.filter(a => a.fecha === hoy);
  ...
  const vacRows = pendVacList.map(v => {
  ...
  <div class="view-header">
    <div class="view-header-left">
      <h1>Dashboard</h1>
      <p>Lunes 23 de Febrero, 2026 · Bienvenido al panel de RHM</p>
    </div>
  ...
  <div class="stats-grid">
    <div class="stat-card">
      ...
      <div class="stat-content">
        ...
        <div class="stat-change pos">↑ 1 nuevo este mes</div>
      </div>
    </div>
    <div class="stat-card">
      ...
      <div class="stat-content">
        ...
        <div class="stat-change pos">↑ ${Math.round(presentes / total * 100)}% asistencia</div>
      </div>
    </div>
    <div class="stat-card">
      ...
      <div class="stat-content">
        ...
        <div class="stat-change neg">↑ 1 vs ayer</div>
      </div>
    </div>
    <div class="stat-card">
  ...
  <div class="grid-2" style="gap:16px;margin-bottom:16px">
    <div class="card">
      <div class="card-header">
        <div><div class="card-title">Asistencia Semanal</div><div class="card-subtitle">Semana 17–23 Feb 2026</div></div>
      </div>
      <div class="card-body"><div class="chart-wrap" style="height:220px"><canvas id="chartAsistencia"></canvas></div></div>
    </div>
    <div class="card">
      <div class="card-header">
        <div><div class="card-title">Distribución por Dpto.</div><div class="card-subtitle">Total ${total} empleados</div></div>
      </div>
  ...
  <div class="grid-2" style="gap:16px">
    <div class="card">
      <div class="card-header">
        <div><div class="card-title">Asistencia de Hoy</div><div class="card-subtitle">23 Feb 2026</div></div>
...
function initDashboard() {
  ...
  const deptCounts = MOCK.departamentos.map(d => d.count);
  const deptNames = MOCK.departamentos.map(d => d.nombre);
  const deptColors = ['#6366f1', '#10b981', '#f59e0b', '#3b82f6', '#ec4899', '#9333ea'];
...
window.aprobarVac = function (id) {
  const v = MOCK.vacaciones.find(x => x.id === id);
  if (v) { v.estado = 'Aprobado'; v.aprobadoPor = currentUser.name; navigate('dashboard'); }
};
window.rechazarVac = function (id) {
  const v = MOCK.vacaciones.find(x => x.id === id);
  if (v) { v.estado = 'Rechazado'; navigate('dashboard'); }
};
